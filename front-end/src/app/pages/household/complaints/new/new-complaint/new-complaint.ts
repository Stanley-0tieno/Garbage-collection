...
export class NewComplaintComponent implements OnInit {
  ...
  /**
   * Only allow complaints on pickups that have actually started/happened.
   * PENDING = not yet assigned → cannot complain about something that hasn't occurred.
   * CANCELLED = no service took place → not applicable.
   * ASSIGNED / COMPLETED = valid for complaints.
   */
  readonly eligiblePickups = computed(() =>
    this.allPickups().filter(p => p.status === 'ASSIGNED' || p.status === 'COMPLETED')
  );

  readonly hasNoEligiblePickups = computed(() => this.eligiblePickups().length === 0);
  ...
  get pickupId()  { return this.form.get('pickupId')!; }

  ngOnInit(): void {
    const userId = this.auth.currentUser()?.id ?? '';
    this.pickups.getMyPickups(userId).subscribe({
      next: data => this.allPickups.set(data),
      error: ()  => {}
    });
  }
  ...
  /** Human-readable label for a pickup in the dropdown */
  pickupLabel(p: PickupRequest): string {
    const date   = p.date ? new Date(p.date).toLocaleDateString('en-KE', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Date TBD';
    const status = p.status === 'ASSIGNED' ? 'In Progress' : 'Completed';
    return `${date} · ${p.wasteType} · ${status}`;
  }
